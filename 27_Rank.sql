...
select *,dense_rank() over(order by cost desc) as 'dense_rank' from restaurants;
select *,row_number() over(order by cost desc) as 'row_number_rank' from restaurants;
...
select *,row_number() over ( partition by city  order by cost desc) as 'row_number_rank' from restaurants;
...
select sum(cost*rating_count) as 'total_revenue'
	from (
			...
			from restaurants) t
where 
  t.rank <= ((select count(*) from restaurants)/100);
--  462,914,000 = 17.5%
...
select sum(cost*rating_count) as 'total_revenue'
	from (
			...
			from restaurants) t
where 
  t.rank <= ((select count(*) from restaurants)/100*20);
